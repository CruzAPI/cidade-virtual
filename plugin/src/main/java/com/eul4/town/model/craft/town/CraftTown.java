package com.eul4.town.model.craft.town;

import com.eul4.town.Main;
import com.eul4.town.model.town.Town;
import com.eul4.town.model.town.TownBlock;
import com.eul4.town.model.town.TownTile;
...
{
	public static final Set<Integer> HOLOGRAMS_ID = new HashSet<>();
	...
	@Override
	public Set<Integer> getHologramsId()
	{
		return tileHolograms.keySet().stream().map(ArmorStand::getEntityId).collect(Collectors.toSet());
	}
	...
	public class CraftTownTile implements TownTile
	{
		...
		private void spawnHologram()
		{
			...
			final TownTile thisTownTile = this;
			...
			new BukkitRunnable()
			{
				...
				public void run()
				{
					...
					EntityArmorStand nmsArmorStand = new EntityArmorStand(worldServer);
					
					nmsArmorStand.locX = veryAwayLocation.getX();
					nmsArmorStand.locY = veryAwayLocation.getY();
					nmsArmorStand.locZ = veryAwayLocation.getZ();
					
					hologram = (ArmorStand) nmsArmorStand.getBukkitEntity();
					
					tileHolograms.put(hologram, thisTownTile);
					HOLOGRAMS_ID.add(hologram.getEntityId());
					
					Bukkit.broadcastMessage("nms hologram id: " + hologram.getEntityId());
					
					boolean freshEntity = worldServer.addEntity(nmsArmorStand);
